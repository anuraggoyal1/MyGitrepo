...
public class NSEFullDataAnalysis {

 /* static String path = "/Users/anuraggoyal/Documents/daily_data/input-nse-full/";

  static Map<String, NSEFullData> pday, tday;
  ...
  public static void main(String[] args) throws IOException {
    ...
    List<String> hSymbols = sym.getHoldingSymbols();
    System.out.println("Total Symbols-" + symbols.size());
    System.out.println("Total Holdings-" + hSymbols.size());
 ...
 //------------------------------------------------------------------------------------------------------------------       
    ...
    for (String s : symbols) {
      if (pday.containsKey(s) && tday.containsKey(s)) {
        if ((pday.get(s).getClose() < pday.get(s).getEma200d())
    ...
    for (String s : symbols) {
      if (pday.containsKey(s) && tday.containsKey(s)) {
        float diff = tday.get(s).getEma200d() - tday.get(s).getClose();
        float diffInPer = (diff *100) / tday.get(s).getEma200d() ;
        
        if (diffInPer > -5 && diffInPer < 5 ) {
    ...
    try {
      ...
      while ((line = reader.readNext()) != null) {
        if (!line[2].isEmpty()) {
          ...
          if(line[0].equalsIgnoreCase("AUROPHARMA")) {
