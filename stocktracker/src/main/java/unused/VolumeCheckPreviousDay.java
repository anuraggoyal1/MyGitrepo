...
public class VolumeCheckPreviousDay {
//	static final Logger logger = Logger.getLogger(VolumeCheckPreviousDay.class);
	...
	public static boolean doVolumePriceCheck(String symbol, Stock stock) {
		if (!alreadyFound.containsKey(symbol)) {
			long previousDayVolumePercent = (previousDay.get(symbol).getVolume() * percent) / 100;
			float diffVol = ((stock.getQuote().getVolume() - previousDay.get(symbol).getVolume())/previousDay.get(symbol).getVolume())*100;
			if (stock.getQuote().getVolume() > previousDayVolumePercent) {
